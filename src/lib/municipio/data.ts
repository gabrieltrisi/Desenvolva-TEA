...
import { type AccessUser, getAccessibleChildrenWhere } from "@/lib/access/children";
...
export async function getMunicipioData(
  ...
  // Consumo de conteúdo: ADMIN vê toda a organização; demais perfis só veem as
  // crianças acessíveis (relação `child`, que exclui views sem criança).
  const contentViewScope: Prisma.ContentViewWhereInput = isAdminScope
    ? { organizationId }
    : { child: childWhere };
  const now = Date.now();
  ...
  const [
    ...
    // Famílias ativas: ADMIN conta todas da org; demais contam responsáveis
    // (ACTIVE) distintos das crianças acessíveis.
    isAdminScope
      ? prisma.user.count({ where: { organizationId, role: "FAMILIA", active: true } })
      : prisma.childGuardianLink
          .findMany({
            where: {
              status: "ACTIVE",
              child: childWhere,
              guardian: { active: true, role: "FAMILIA" },
            },
            select: { guardianUserId: true },
            distinct: ["guardianUserId"],
          })
          .then((r) => r.length),
    // Profissionais: ADMIN conta todos da org; demais contam terapeutas
    // distintos das crianças acessíveis.
    isAdminScope
      ? prisma.user.count({ where: { organizationId, role: "PROFISSIONAL", active: true } })
      : prisma.child
          .findMany({
            where: { AND: [childWhere, { therapistId: { not: null } }] },
            select: { therapistId: true },
            distinct: ["therapistId"],
          })
          .then((r) => r.length),
    prisma.evolutionRecord.count({
      where: { child: childWhere, date: { gte: filters.start, lte: filters.end } },
    }),
    prisma.contentView.count({
      where: { ...contentViewScope, viewedAt: { gte: filters.start, lte: filters.end } },
    }),
    prisma.child.aggregate({ where: childWhere, _avg: { overallEvolution: true } }),
    // ReportEmission não tem vínculo criança/município — fora do escopo ADMIN
    // não há como filtrar com segurança (ver checkpoint). Interino seguro: 0.
    isAdminScope
    ...
    prisma.evolutionRecord.findMany({
      where: { child: childWhere, date: { gte: sixMonthsAgo } },
      select: {
        date: true,
        communication: true,
        social: true,
    ...
    prisma.contentView.groupBy({
      by: ["childId"],
      where: { ...contentViewScope, childId: { not: null }, viewedAt: { gte: filters.start, lte: filters.end } },
      _count: { _all: true },
    }),
  ]);
...
export async function getMunicipioComparison(
  ...
  const [records, views, emissions] = await Promise.all([
    prisma.evolutionRecord.findMany({
      where: { child: childWhere, date: { gte: prevStart, lte: end } },
      select: {
        date: true,
        childId: true,
        communication: true,
    ...
    prisma.contentView.findMany({
      where: {
        ...(isAdminScope ? { organizationId } : { child: childWhere }),
        viewedAt: { gte: prevStart, lte: end },
      },
      select: { viewedAt: true },
    }),
    // ReportEmission sem vínculo criança/município: 0 fora do escopo ADMIN.
    isAdminScope
      ? prisma.reportEmission.findMany({
          where: { organizationId, emittedAt: { gte: prevStart, lte: end } },
          select: { emittedAt: true },
        })
      : Promise.resolve([] as { emittedAt: Date }[]),
  ]);
