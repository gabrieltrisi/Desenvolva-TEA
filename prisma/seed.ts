...
import { seedMunicipalitiesDemo } from "./municipalities-demo";
...
async function main() {
  ...
  // --- Demonstração multi-prefeitura (Fase 6): municípios + vínculos + famílias ---
  const muniDemo = await seedMunicipalitiesDemo(prisma, org.id, passwordHash);
  console.log(
    `   + Multi-prefeitura: ${muniDemo.municipalities} municípios, ${muniDemo.prefeituraUsers} gestores, ` +
      `${muniDemo.childrenRedistributed} crianças redistribuídas, ${muniDemo.pendingCreated} PENDING, ` +
      `${muniDemo.familiesCreated} famílias.`,
  );
